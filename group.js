var debugrmNE = false;
group = function (minterms, dontcares, dim, variables, out) {
...
reduceGroupedTerms = function (groupTerms, dim) {
    var mistakeCount = 0;
    var reducedGroup = new Array(dim + 1);
    for (var i = 0; i < reducedGroup.length; i++) {
        reducedGroup[i] = new Array();
    }
    for (var i = 0; i < groupTerms.length - 1; i++) {
        for (var j = 0; j < groupTerms[i].length; j++) {
            for (var k = 0; k < groupTerms[i + 1].length; k++) {
                var aStr = groupTerms[i][j].replace(/-/g, '0');
                var bStr = groupTerms[i + 1][k].replace(/-/g, '0');
                var a = parseInt(aStr, 2);
                var b = parseInt(bStr, 2);
                var abStr = leftPadString((b - a).toString(2), dim, '0');
                if (differenceCount(groupTerms[i][j],groupTerms[i+1][k]) == 1) {
                    var newStr = groupTerms[i][j].substr(0, abStr.indexOf('1')) + '-' + aStr.substr(abStr.indexOf('1') + 1);
                    if ((newStr.match(/-/g) || []).length > (groupTerms[i][j].match(/-/g) || []).length)
...
//funksioni me parameter termat qe do futen nga perdoruesi, kushtet indiferente dhe variabel per reduktimin e grupeve te termave
...
//funksioni qe ben te mundur grupimin e termave ne harten Karno
...
removeNonEssentials = function(essentials, dontcares) {
    if(debugrmNE) console.log('Esencialet:', essentials);
    var totalCoverage = [];
    ...
    for (var i = essentials.length-1; i >= 0; i--) {
        ...
        if(debugrmNE) console.log('Mbulim total:', totalCoverage);
        if(debugrmNE) console.log('Mbulim i vetem: ', currCov);
        var added = false;
        for (var j = 0; j < currCov.length; j++) {
            ...
            if(debugrmNE) console.log('Elementet esencial te reduktuar: ', reducedEssentials);
            if(debugrmNE) console.log('Mbulimi total:', totalCoverage);
        }
    }
    if(debugrmNE) console.log('Elementet esencial te reduktuar:', reducedEssentials);
...
essentialstoGenString = function(essentials, variables, output) {
 ...
 essentialsToVHDLString = function(essentials, variables, output) {
    ...
    for (var i = 0; i < essentials.length; i++) {
        ...
        for (var j = 0; j < essentials[i].length; j++) {
            if (essentials[i][j] == '0') {
                string += 'jo ' + variables[j] + ' dhe ';
                subtract = 5;
            }
            else if (essentials[i][j] == '1') {
                string += variables[j] + ' dhe ';
                subtract = 5;
            }
        }
        string = string.substr(0, string.length - subtract) + ') ose (';
    }
...
essentialsToVerilogString = function(essentials, variables, output) {
    if (essentials.length == 0)
        return "cakto " + output + ' = 0;';
    else if (essentials.length == 1 && (essentials[0].match(/-/g)||[]).length == essentials[0].length)
        return "cakto " + output + ' = 1;';
    var string = 'cakto ' + output + ' = (';
    for (var i = 0; i < essentials.length; i++) {
        var subtract = 0;
        for (var j = 0; j < essentials[i].length; j++) {
            if (essentials[i][j] == '0') {
