...
// Ngarkimi i dritares dhe ridemsionimi i saj
...
mapContinue = function() {
    ...
    var strings = group(minterms, dontcares, dimension, variables, output);
...
// Verifikimi i te dhenave te futur nga perdoruesi

assertSetupInputs = function(d, v, o) {
    if (isNaN(d) || (parseInt(d, 10) != d)) {
        alert("Inputet duhet te jene numra.");
        return false;
    }
    else if ( parseInt(d,10) <= 1) {
        alert("Numri i inputeve duhet te jete me i madh se 1.");
        return false;
    }
    if (v.length != parseInt(d)) {
        alert("Numri i variablave dhe inputeve duhet te jete i njejte");
        return false;
    }
    return true;
}
...
mapClear = function() {
    var clear = confirm("Kjo do te fshije te gjitha te dhenat");
    if (clear) {
